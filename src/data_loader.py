# DataLoader
# This module provides classes for loading and processing data from Excel and  JSON files.
            ...
            try:
                ...
                if len(data) > 0:
                    ...
                    if sheet_name == "EDA_rs":
                        ...
                        self.dataInfo.edaDataInfo.nb_channels = 1
                        self.dataInfo.edaDataInfo.session_nb_samples = len(signal_data)
                        if sampling_rate > 0:
                        ...
                        signal_data = signal_data.astype(float)
                        self.dataInfo.setHasBVP(True)
                        self.dataInfo.bvpDataInfo.setSamplingRate(sampling_rate)
                        self.dataInfo.bvpDataInfo.nb_channels = 1
                        self.dataInfo.bvpDataInfo.rs_nb_samples = len(signal_data)
                        if sampling_rate > 0:
                            self.dataInfo.bvpDataInfo.rs_duration = len(signal_data) / sampling_rate
                        else:
                            self.dataInfo.bvpDataInfo.rs_duration = 0
                        self.dataInfo.bvpDataInfo.file_path = self.filename
                        ...
                        self.dataInfo.bvpDataInfo.setSamplingRate(sampling_rate)
                        self.dataInfo.bvpDataInfo.nb_channels = 1
                        self.dataInfo.bvpDataInfo.session_nb_samples = len(signal_data)
                        if sampling_rate > 0:
                            self.dataInfo.bvpDataInfo.session_duration = len(signal_data) / sampling_rate
